// Updated MockInterview.jsx with Gemini feedback and score breakdown
import React, { useEffect, useRef, useState } from 'react';
import { FaMicrophone, FaKeyboard, FaRobot, FaCamera, FaVideoSlash, FaVideo, FaPlay } from 'react-icons/fa';
...
const Interview = () => {
  ...
  const handleSubmit = async () => {
    ...
    setScore(null);
    try {
      const res = await axios.post('http://localhost:8001/evaluate_answer', {
        question,
        answer
      });
  ...
  const scoreLabels = ['Clarity', 'Relevance', 'Structure', 'Confidence', 'Keyword Usage'];
    ...
    <div className="p-6 min-h-screen bg-gradient-to-br from-white to-indigo-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="relative">
          ...
          >
            {cameraOn ? <><FaVideoSlash /> Camera Off</> : <><FaVideo /> Camera On</>}
          </button>
        </div>
          ...
          <div className="p-4 bg-white rounded-xl shadow">
            ...
            <div className="flex gap-4 flex-wrap mt-4">
              <button
                ...
                className={`px-4 py-2 rounded ${mode === 'voice' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}
              >
                <FaMicrophone /> Voice
              </button>
              <button
                onClick={() => setMode('text')}
                className={`px-4 py-2 rounded ${mode === 'text' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}
              >
                <FaKeyboard /> Text
              </button>
              <button
      ...
      {feedback && !loading && (
        <div className="mt-8 p-6 bg-white rounded-xl shadow-lg">
          <h4 className="text-2xl font-bold text-indigo-700 mb-2">AI Feedback</h4>
          <p className="text-gray-700 whitespace-pre-line mb-4">{feedback}</p>

          {score && (
            <div className="mt-4">
              <h5 className="text-lg font-semibold text-indigo-600 mb-2">Score Breakdown (out of 10)</h5>
              <ul className="text-gray-800 text-sm list-disc pl-6">
                {scoreLabels.map((label, index) => (
                  <li key={label}><strong>{label}:</strong> {score[index]}</li>
                ))}
              </ul>
            </div>
          )}
